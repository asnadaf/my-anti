...
export async function sendLicenseKeyEmail({ to, orderId, licenseKeys }) {
  ...
  const html = `
    ...
    <p>Please save this order ID for future reference.</p>
    <h2>License Keys:</h2>
    <ul>
      ${licenseKeys.map(key => `<li><code>${key}</code></li>`).join('')}
    </ul>
    <p>If you have any questions, please contact our support team with your order ID.</p>
  `;
  const text = `
    ...
    If you have any questions, please contact our support team with your order ID.
  `;
