...
export async function fetchProductBySlug(slug) {
  ...
  try {
    // If we're on the client side, use the API endpoint
    if (!isServer) {
    ...
    // Server-side: connect to database and fetch directly
    await dbConnect();
...
export async function fetchFilteredProducts(options = {}) {
  ...
  try {
    ...
    if (!isServer) {
      console.log('Client-side filtering - using API endpoint');
      try {
