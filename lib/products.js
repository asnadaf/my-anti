...
export async function fetchFilteredProducts(options = {}) {
  try {
    ...
    // Remove status filter if it doesn't exist in the schema
    try {
      const productSchemaKeys = Object.keys(await Product.findOne().lean() || {});
      if (!productSchemaKeys.includes('status')) {
        delete filter.status;
      }
    } catch (e) {
      console.warn('Could not check schema, removing status filter', e);
      delete filter.status;
    }
